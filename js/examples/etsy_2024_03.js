// KNOWN ISSUE: This won't download videos due to URL format difference.
let getItmInfo = function(){
  return itm_url_split = document.location.href.split('?')[0].split('/');
}
...
let downloadName = function(url, i){
  ...
  let download_name = ["etsy",getItmNum(),getItmTitle(),getFilenameName(filename),i].join(" ");
...
let grabImageUrls = function(){
  let img_urls = [];
  let pics = document.getElementsByClassName("listing-page-image-carousel-component")[0].getElementsByClassName("carousel-image");
  if (pics.length > 0){
    for(var i=0;i<pics.length;i++){
      img_urls.push(pics[i].dataset.srcZoomImage || pics[i].src);
    }
  }
  return img_urls;
}
...
let processImgUrls = function(img_urls){
  ...
  if(img_urls_buffer.length>0){
    for (let i=0; i<img_urls_buffer.length; i++){
      let img_filename = img_urls_buffer[i].split('/').pop();
      let new_img_filename = img_filename.split('?')[0];
      new_img_filename = new_img_filename.replace(/^il_[^x.]+x[^x.]+./,"il_fullxfull.");
      img_urls_buffer[i] = img_urls_buffer[i].replace(img_filename, new_img_filename);
      img_urls_buffer[i] = img_urls_buffer[i].replace(/\/d\//,"/r/");
    }
  }
