let getItmNum = function(){
  return document.location.href.split('?')[0].split('/')[4];
}
...
let downloadName = function(url, i){
  ...
  let download_name = ["gearhuman",getItmNum(),getFilenameName(filename),i].join(" ");
...
let grabImageUrls = function(){
  let img_urls = [];
  var pics = document.querySelectorAll(".swiper-slide.relative.cursor-pointer.media-type-image.swiper-slide-visible img");
  if (pics.length > 0){
