// KNOWN ISSUE: This won't work if there are more than 5-6 images. A
// special script might be needed.
let getItmNum = function(){
  return document.location.href.split('?')[0].split('/')[5];
}

let getItmTitle = function(){
  let item_title = document.getElementById("main-title").innerHTML;
...
let downloadName = function(url, i){
  ...
  let download_name = ["walmart",getItmNum(),getItmTitle(),getFilenameName(filename),i].join(" ");
...
let grabImageUrls = function(){
  let img_urls = [];
  let pics = document.querySelectorAll("[data-testid='vertical-carousel-container'] img");
  if (pics.length > 0){
...
let processImgUrls = function(img_urls){
  ...
  for(let i=0;i<img_urls_buffer.length;i++){
    img_urls_buffer[i] = img_urls_buffer[i].split("?")[0];
  }
